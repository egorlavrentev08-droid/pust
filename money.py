...
# Версия: 3.0.0 (с новой системой расходников)
...
from config import (
    ...
    ENERGY_DRINKS, REDUCERS, get_energy_bonus, get_reducer_bonus
)
...
from utils import (
    ...
    log_user_action, escape_markdown
)
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text += "• 💊 Аптечка (125)\n"
        if user.level >= 5:
            text += "• 🔍 Металлоискатель (10000) — 10% шанс найти сундук\n"
        text += "\n"
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Купить предмет (включая новые энергетики и редукторы)"""
    if not context.args:
    ...
    try:
        ...
        if item == 'броня5' and user.level < 50:
            ...
            return
        if item == 'металлоискатель' and user.level < 5:
            await update.message.reply_text("❌ Металлоискатель доступен с 5 уровня!")
            return
        ...
        if item in ENERGY_DRINKS:
            ...
            add_item_to_inventory(user, 'металлоискатель', count)
        elif item == 'ружье':
        ...
        # Добавляем новые предметы в словарь названий
        for key, data in ENERGY_DRINKS.items():
        ...
        # Показываем остаток лимита (только для старых лимитированных товаров)
        if item in SHOP_LIMITS:
            limits = context.bot_data.get('shop_limits', {})
            available = limits.get(item, 0)
...
async def sell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sell_prices = {
            ...
            'аптечка': 100, 'металлоискатель': 8000
        }
...
async def equip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Экипировать предметы (без изменений)"""
...
async def craft(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Добавляем рецепты для новых энергетиков
        for key, data in ENERGY_DRINKS.items():
            recipes[key] = {
                'rf': int(data['price'] / 2),
                'rc': int(data['price'] / 2),
        ...
        # Добавляем рецепты для новых редукторов
        for key, data in REDUCERS.items():
            recipes[key] = {
                'rf': int(data['price'] / 2),
                'rc': int(data['price'] / 2),
