...
async def clan_give(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log_user_id = target.user_id
        log_username = target.username
        log_sender_id = user.user_id
        log_sender_name = user.username
        
        session.commit()
        
        # Логируем выдачу
        log_user_action(
            user_id=log_sender_id,
            username=log_sender_name,
            action='clan_give',
            amount_rc=-amount,
            item=f"{username} ({clan.name})"
        )
        log_user_action(
            user_id=log_user_id,
            username=log_username,
...
async def clan_upgrade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log_user_action(
            user_id=log_user_id,
            username=log_username,
            action=f'clan_upgrade_{upgrade}',
            amount_crystals=-cost,
            item=clan.name
        )
...
async def radion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Логируем активацию радио
        log_user_action(
            user_id=log_user_id,
            username=log_username,
            action='radio_activate',
            item=code
        )
...
async def radio_register_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Регистрация группы для радио (без уведомления)"""
