...
async def aradio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'give':
        ...
        if len(context.args) < 2:
            await update.message.reply_text("❌ /aradio add [ссылка/ID]")
            return
        identifier = context.args[1]
        groups = context.bot_data.get('radio_groups', set())
        
        # Пробуем получить chat_id
        chat_id = None
        if identifier.startswith('@'):
            ...
            await update.message.reply_text("❌ Укажите ID чата или ссылку (например, @username)")
            return
        ...
        if len(context.args) < 2:
            await update.message.reply_text("❌ /aradio clear [ссылка/ID]")
            return
        identifier = context.args[1]
        groups = context.bot_data.get('radio_groups', set())
        ...
        if chat_id in groups:
            ...
            await update.message.reply_text(f"❌ Группа `{chat_id}` не найдена в списке радио", parse_mode='Markdown')
