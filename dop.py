# dop.py - Дополнительные механики
# Версия: 1.2.0 (финальная)
...
from config import logger, MAX_LEVEL, get_exp_for_level, get_energy_bonus, get_reducer_bonus
from core import send_to_private
from database import Session, User
from utils import escape_markdown, safe_log_user_action, get_item_count
...
async def use_metal_detector(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        available = get_item_count(user, 'металлоискатель')
        if available == 0:
            await update.message.reply_text(
                "🔍 *У вас нет металлоискателя!*\n\n"
                "Купить можно в магазине: `/buy металлоискатель`",
                parse_mode='Markdown'
            )
            return
        ...
        chance = min(25, 10 * chest_mult)
        cooldown_hours = random.randint(3, 18)
        
        user.last_metal_detector = datetime.now()
        user.last_metal_detector_duration = cooldown_hours
        
        if random.random() < chance / 100:
            chest_type = random.choice(['common', 'rare', 'epic', 'mythic', 'legendary'])
            chest_names = {
                'common': '🟢 Обычный',
                'rare': '🔵 Редкий',
                'epic': '🟣 Эпический',
