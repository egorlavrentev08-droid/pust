...
# Импорты из utils
from utils import add_item_to_inventory, remove_item_from_inventory, get_item_count, log_user_action
    ...
    try:
        ...
        if resource == 'RC':
            ...
            log_user_action(
                ...
                action='admin_give',
                amount_rc=amount,
                item=f"{resource} от {update.effective_user.username}"
            )
        elif resource == 'RF':
            ...
            if user.clan_id:
                ...
                if clan:
                    ...
                    log_user_action(
                        ...
                        username=log_username,
                        action='admin_give',
                        amount_crystals=amount,
                        item=f"{resource} в казну клана {clan.name} от {update.effective_user.username}"
                    )
                    ...
                    log_user_action(
                        ...
                        action='admin_give',
                        amount_crystals=amount,
                        item=f"{resource} лично от {update.effective_user.username}"
                    )
    ...
    try:
        ...
        if action == 'give':
            ...
            log_user_action(
                user_id=log_user_id,
                username=log_username,
                action='admin_item_give',
                item=f"{item}x{count} от {update.effective_user.username}"
            )
...
async def advice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tips = {
        ...
        'предметы': "📦 *ПРЕДМЕТЫ*\n━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                   ...
                   "• энергетик (250 RC) — +10% выживание, +5% RC, +25% RF, x2 кристаллы (6ч)\n"
                   "• редуктор (1250 RC) — ускоряет восстановление сбора вдвое (3д)\n\n"
                   "*⚙️ УЛУЧШЕНИЯ*\n"
                   "• /use энергетик [кол-во] — активировать\n"
                   "• /use редуктор [кол-во] — активировать\n\n"
                   "💡 Предметы можно купить в `/shop`, продать через `/sell`, а экипировать через `/equip`",
    }
