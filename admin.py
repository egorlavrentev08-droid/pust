...
async def admin_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    valid_items = old_items + new_energies + new_reducers
    
    if item not in valid_items:
        await update.message.reply_text(f"❌ Неизвестный предмет. Доступны: {', '.join(valid_items[:15])}...")
        return
...
async def admin_players(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Список всех игроков (админ) — исправленная версия с закрытием сессии"""
    if not await is_admin(update, context):
        await update.message.reply_text("❌ Нет прав!")
        return
    session = Session()
...
async def check_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Проверка действий пользователя (админ) — с экранированием"""
    if not await is_admin(update, context):
        await update.message.reply_text("❌ Нет прав!")
        return
