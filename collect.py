...
from utils import (get_equipped, get_item_count, add_item_to_inventory,
...
async def collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user.energy_drink_until and user.energy_drink_until > now:
            rc_gain = int(rc_gain * 1.05)
            fragment_gain = int(fragment_gain * 1.25)
            crystal_gain = int(crystal_gain * 2)
        ...
        # ДОСТИЖЕНИЯ — БЕЗ СЕССИИ! ТОЛЬКО ПРОВЕРКА
        ...
        session.commit()  # <-- ОДИН КОММИТ, ВСЁ СОХРАНЯЕТСЯ
...
async def hunt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if survived:
            msg = (f"🏹 *Охота в Пустоши*\n\nВы крадётесь с {weapon_name}. {armor_name} защищает вас.\n\n"
