...
# Импорты из utils
from utils import get_equipped, get_item_count, add_item_to_inventory, remove_item_from_inventory, apply_class_bonus, calculate_survive_chance, check_achievements, log_user_action, save_equipped
...
async def collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Сохраняем данные для лога ДО session.commit()
        log_user_id = user.user_id
        log_username = user.username
        log_rc = rc_gain
        log_rf = fragment_gain
        log_crystals = crystal_gain
        
        session.commit()
        
        # ===== ЛОГИРОВАНИЕ (после commit, используем сохранённые данные) =====
        log_user_action(
            user_id=log_user_id,
...
async def hunt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log_user_id = user.user_id
        log_username = user.username
        log_rf = reward_rf if survived else 0
