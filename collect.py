...
async def collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сбор ресурсов с учётом новых энергетиков"""
    ...
    try:
        ...
        # Логируем действие
        log_user_action(log_user_id, log_username, 'collect',
...
async def hunt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Охота с учётом новых классов и энергетиков"""
    user_id = update.effective_user.id
    session = Session()
    try:
        user = session.query(User).filter_by(user_id=user_id).first()
        ...
        if encounter_type == 'mutant':
            ...
            # Определяем противника в зависимости от класса
            ...
            if roll < human_chances[0]:
                ...
                target_name = f"⚔️ *Толпа {enemy_class.capitalize()}ов*"
                target_desc = "несколько врагов, застигнутых врасплох"
            else:
                reward_rf, target_size = 1000, 'караван'
                target_name = f"🚚 *Караван {enemy_class.capitalize()}ов*"
        ...
        if target_level >= 2:
            ...
            if random.random() * 100 > survive_chance:
                ...
                if not survived:
                    user.deaths += 1
                    eq = get_equipped(user)
                    if eq.get('armor'):
                        add_item_to_inventory(user, eq['armor'], 1)
                    if eq.get('weapon'):
                        add_item_to_inventory(user, eq['weapon'], 1)
                    save_equipped(user, {'armor': None, 'weapon': None})
                    reward_rf = reward_exp = 0
        ...
        log_user_id, log_username, log_rf = user.user_id, user.username, reward_rf if survived else 0
        session.commit()

        if survived and reward_rf > 0:
            log_user_action(log_user_id, log_username, 'hunt', amount_rf=log_rf)
    ...
    if not context.args:
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"🎪 `market` — Рынок (предметы 10%)", parse_mode='Markdown')
        finally:
            session.close()
        return
